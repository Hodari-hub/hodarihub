...
//////////////////////////////////////////////////////////////////
    ...
    route.get("/profile/:id",isAuth,(req,res)=>{
        //if user is not the admin dont allow them to access othr user profile
        if(req.cookies.userType!="admin"&&req.params.id!=req.cookies.userId){ $check_id=req.cookies.userId;}
        else{ $check_id=req.params.id; }
        //run the query to get the details for this user id
        conn.query(`SELECT * FROM base_members WHERE m_id='${$check_id}'`, 
            function (error, results) {
                ...
                if(results.length){
                    ...
                    let m_phone_number=results[0].m_phone_number,m_sec_number=results[0].m_sec_number;
                    let m_email=results[0].m_email,m_type=results[0].m_type.toUpperCase(),numpost=0,numrt=0,reach=0,impres=0;
                    ...
                    numpost=syncSql.mysql(config, `SELECT COALESCE(SUM(stats_id),0) AS numpost FROM twitter_stats  LEFT JOIN bots ON  twitter_stats.owner_id=bots.bots_id WHERE bots.owner_id='${m_id}' AND twitter_stats.post_type='POST OR COMMENT'`);
                    if(numpost.data.rows.length){numpost=numpost.data.rows[0].numpost;}
                    numrt=syncSql.mysql(config, `SELECT COALESCE(SUM(stats_id),0) AS numrt FROM twitter_stats  LEFT JOIN bots ON  twitter_stats.owner_id=bots.bots_id WHERE bots.owner_id='${m_id}' AND twitter_stats.post_type='RT'`);
                    if(numrt.data.rows.length){numrt=numrt.data.rows[0].numrt;}
                    impres=syncSql.mysql(config, `SELECT COALESCE(SUM(impression),0) AS impresion FROM twitter_stats  LEFT JOIN bots ON  twitter_stats.owner_id=bots.bots_id WHERE bots.owner_id='${m_id}' AND twitter_stats.post_type='POST OR COMMENT'`);
                    if(impres.data.rows.length){impres=impres.data.rows[0].impresion;}
                    reach=syncSql.mysql(config, `SELECT COALESCE(SUM(reach),0) AS reach FROM twitter_stats  LEFT JOIN bots ON  twitter_stats.owner_id=bots.bots_id WHERE bots.owner_id='${m_id}' AND twitter_stats.post_type='POST OR COMMENT'`);
                    ...
                    conn.query(`SELECT * FROM bots LEFT JOIN twitter_stats ON bots.bot_id=twitter_stats.owner_id WHERE bots.owner_id='${m_id}' ORDER BY bots.bot_name`,(err,rs)=>{
                        ...
                        if(rs.length){
                            for(let i =0; i<rs.length; i++){
                                let RT =syncSql.mysql(config, `SELECT COALESCE(SUM(stats_id),0) AS rt FROM twitter_stats  LEFT JOIN bots ON  twitter_stats.owner_id=bots.bots_id WHERE twitter_stats.owner_id='${rs[i].bots_id}' AND twitter_stats.post_type='RT' `).data.rows[0].rt;
                                let T =syncSql.mysql(config, ` SELECT COALESCE(SUM(stats_id),0) AS tw FROM twitter_stats  LEFT JOIN bots ON  twitter_stats.owner_id=bots.bots_id WHERE twitter_stats.owner_id='${rs[i].bots_id}' AND twitter_stats.post_type='POST OR COMMENT' `).data.rows[0].tw;
                                let I =syncSql.mysql(config, ` SELECT COALESCE(SUM(impression),0) AS impres FROM twitter_stats  LEFT JOIN bots ON  twitter_stats.owner_id=bots.bots_id WHERE twitter_stats.owner_id='${rs[i].bots_id}' AND twitter_stats.post_type='POST OR COMMENT' `).data.rows[0].impres;
...
////////////////////////////END OF USERS & PROFILE ISSUE////////////////////////////////
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX


/////////////////////////////////////////////////////////////////////////////////
////////////////////EMAIL & PLATFORM & SOCIAL MEDIA ISSUES /////////////////////
///////////////////////////////////////////////////////////////////////////////

    //insert new media in the list 
    ...
    //View all media list available in the database
            ...
            if(p_res.length){
                for(let i =0; i<p_res.length; i++){
                    plt+=`<tr id='tr_${p_res[i].plat_id}'>
                            <td>${i+1}</td> <td>${p_res[i].platform_name}</td>
                            <td>${p_res[i].number_used}</td> <td>${p_res[i].email_used}</td>
...
////////////////////////////END OF EMAIL & PLATFORM & SOCIAL MEDIA ISSUES////////////////////////////////
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
////////////////////////////////////TWITTER AUTOMATION//////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////
    route.get("/new_tone_keyword",isAuth,(req,res)=>{res.render("forms",{pageTitle:"ADD NEW KEYWORD", user_name:req.cookies.userName, form:"new_tone_keyword" });});

    route.get("/new_rt_listener",isAuth,(req,res)=>{
    ...
    route.get("/rt_automation",isAuth,(req,res)=>{
        ...
        conn.query(`SELECT * FROM key_bystander LEFT JOIN bots ON key_bystander.bot_id=bots.bot_id JOIN base_members ON key_bystander.owner_id=base_members.m_id WHERE ${checker}`, 
            function (error, results) {
                let bystanders="";
                if(error) {res.render("twitter_rt_automation", {pageTitle:"BYSTANDERS",user_name:req.cookies.userName,bystanders:[bystanders]});}; 
                if(results.length){
                    for(let i =0; i<results.length; i++){
                        bystanders+=`<tr id='tr_${results[i].key_id}'> <td>${i+1}</td><td>${results[i].bot_name}</td><td>${results[i].keyword}</td><td>${results[i].m_name}</td>
                                        <td><span class="badge badge-danger delete_automate_key" role='button' data-id='${results[i].key_id}'>Delete</span></td>
                                    </tr>`;
                    }
                    res.render("twitter_rt_automation", {pageTitle:"BYSTANDERS",user_name:req.cookies.userName,bystanders:[bystanders]});
                }
                else{
                    res.render("twitter_rt_automation", {pageTitle:"BYSTANDERS",user_name:req.cookies.userName,bystanders:[bystanders]});
                }
    ...
    route.get("/new_rt_key",isAuth,(req,res)=>{
        if(req.cookies.userType=="admin"){checker=`1`;}else{checker=`owner_id='${req.cookies.userId}' `;}
        conn.query(`SELECT * FROM bots WHERE ${checker}`, 
            function (error, results) {
                let bots="";
                if(error) { res.render("forms",{pageTitle:"ADD NEW BYSTANDER",user_name:req.cookies.userName, form:"automate", bots:[bots]});}
                if(results.length){
                    for(let i =0; i<results.length; i++){
                        let bot_name=results[i].bot_name,bot_id=results[i].bot_id;
                        let isSelected; if(i==0){ isSelected="selected";}else{ isSelected="";}
                    ...
                    res.render("forms",{pageTitle:"ADD NEW BYSTANDER",user_name:req.cookies.userName, form:"automate", bots:[bots]});
                }
                else{res.render("forms",{pageTitle:"ADD NEW BYSTANDER",user_name:req.cookies.userName, form:"automate", bots:[bots]});}
            });
    });

    //retweet from specific user
    route.get("/rt_from_specific_user",isAuth,(req,res)=>{
            ...
            if(rs.length){
                for(let i =0; i<rs.length; i++){
                    listeners+=`<tr id='tr_${rs[i].ky_id}'><td>${i+1}</td><td>${rs[i].bot_name}</td><td>${rs[i].keyword}</td><td>${rs[i].from_author_name}</td><td>${rs[i].from_author_id}</td>
                                    <td><span class="badge badge-danger delete_rt_keywords" role='button' data-name='${rs[i].keyword}' data-id='${rs[i].ky_id}'>Delete</span></td>
                                </tr>`;
                }
                res.render("retweet_from_specific",{pageTitle:"LISTENERS", user_name:req.cookies.userName,listener:[listeners]});
            }
    ...
    //this is created as a helper to insert data to the database
    route.get("/single_tweet_stats",isAuth,(req,res)=>{
        let bots="";
        conn.query(`SELECT * FROM bots WHERE medianame='twitter'`,(err,bots_res)=>{
            if(err) throw err;
            if(bots_res.length){
    ...
    route.get("/influensor_page_stats",isAuth,(req,res)=>{
        res.render("influencers_page_stats",{pageTitle:"INFLUENSOR STATISTIC", user_name:req.cookies.userName});
    });
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
///////////////////////////////////////////END OF TWITTER AUTOMATION/////////////////////////////////////////
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //tonaliy counter
    route.get("/standby_counter",isAuth,(req,res)=>{
        ...
        conn.query(`SELECT * FROM daily_tone WHERE ${checker}`, 
            function (error, results) {
                ...
                if(error) {res.render("/tonality_standby_counter",{pageTitle:"TONE COUNTER",user_name:req.cookies.userName,words:[words]});}
                ...
                if(results.length){
                    ...
                    res.render("tonality_standby_counter",{pageTitle:"TONE COUNTER",user_name:req.cookies.userName,words:[words]});
                }
                else{
                    res.render("tonality_standby_counter",{pageTitle:"TONE COUNTER",user_name:req.cookies.userName,words:[words]});
                }
    ...
    //view tonality data
    route.get("/tonality",isAuth,(req,res)=>{
        ...
        conn.query(`SELECT * FROM message_trend WHERE 1`, function (error, results) {
            ...
            if(results.length){
                for(let i =0; i<results.length; i++){
                    ...
                    messanger=results[i].messanger, datemade=results[i].datemade.toISOString().slice(0, 10);
    ...
    route.get("/tonality_member",isAuth,(req,res)=>{
        //get member list from database and assign them privillage
        conn.query(`SELECT * FROM base_members WHERE 1`,  function(mem_error, mem_results) {
            if(mem_error){console.log(`we have an error in the on sql line number 731 ${mem_error}`);}
            else{
                let members_opt="";
                if(mem_results.length){for(let i=0; i<mem_results.length; i++){members_opt+=`<option value='${mem_results[i].m_id}'>${mem_results[i].m_name}</option>`;}}

                //get available member 
                conn.query(`SELECT * FROM tonality_members LEFT JOIN base_members ON base_members.m_id=tonality_members.member_id WHERE 1`,(m_err,m_req)=>{
                    if(m_err){console.log(`I got this error on line: ${m_err}`);}
                    else{
                        let tone_member="";
                        if(m_req.length){
                            for(let i=0; i<m_req.length; i++){
                                tone_member+=`<tr class='tr' id='tr_${m_req[i].tonality_mem_id}'>
                                                <td>${i+1}</td> <td>${m_req[i].m_name}</td>
                                                <td><span class="badge badge-danger delete_member" role='button' data-mname='${m_req[i].m_name}' data-id='${m_req[i].tonality_mem_id}'>Delete</span></td>
                                            </tr>`;
                            }
                            res.render("tonality_member",{pageTitle:"TONALITY MEMBER", user_name:req.cookies.userName,tone_members:tone_member,member_opt:members_opt});
                        }
                        else{
                            res.render("tonality_member",{pageTitle:"TONALITY MEMBER", user_name:req.cookies.userName,tone_members:tone_member,member_opt:members_opt});
                        }
    ...
    route.get("/twitter_tone",isAuth,(req,res)=>{
        res.render("tonality_twitter",{pageTitle:"TWITTER TONALITY", user_name:req.cookies.userName,});
    });

    route.get("/youtube_tone",isAuth,(req,res)=>{
        res.render("tonality_youtube",{pageTitle:"YOUTUBE TONALITY", user_name:req.cookies.userName,});
    });


    route.get("/youtube_comment_tone",isAuth,(req,res)=>{
        res.render("tonality_youtube_counter",{pageTitle:"YOUTUBE COMMENT TONALITY", user_name:req.cookies.userName,});
    });

    //pending data tone
    route.get("/pending_data",isAuth,(req,res)=>{
        let pending="";

        //if user is not equal to tonality member
        if(!req.cookies.isToneMember){
            res.render("dashboard",{pageTitle:"DASHBOARD",user_name:req.cookies.userName});
            return
        }

        //get pending data from the table 
        conn.query(`SELECT * FROM twitter_temp_tone WHERE tonality_mem_id='${req.cookies.toneMemId}'`,(err,q_res)=>{
            if(err) throw err;
            if(q_res.length){
                for(let i=0; i<q_res.length; i++){
                    pending+=`<tr class="tr_item" id='tr_item_${q_res[i].tonality_mem_id}' title='${q_res[i].caption}'>
                                    <td class="date_tr">
                                        <span class="action_span border">
                                            <i id="delet_btn_${q_res[i].tonality_mem_id}" role="button" class="fa fa-trash  fa-1x delet_tr" title="Delete This Item" aria-hidden="true" data-dataid="${q_res[i].tonality_mem_id}"></i>
                                        </span> 
                                        ${q_res[i].date_created}
                                    </td>
                                    <td>${q_res[i].page_name} / ${q_res[i].caption}</td><td class='positive'>${q_res[i].positive}</td><td class='negative'>${q_res[i].negative}</td><td class='neutral'>${q_res[i].neutral}</td><td class='unrelated'>${q_res[i].unrelated}</td>
                                </tr>`;
                }
                res.render("tonality_pending_data",{pageTitle:"PENDING DATA", user_name:req.cookies.userName,tone: pending});
            }
            else{
                res.render("tonality_pending_data",{pageTitle:"PENDING DATA", user_name:req.cookies.userName,tone: pending});
            }
