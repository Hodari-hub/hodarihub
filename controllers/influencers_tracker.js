...
var getInfluencersStats = async (BareToken,botid,next_token,fromdate,todate)=>{
  ...
  for(let i =0; i<bothTR.body.meta.result_count; i++){
    ...
    let isRaisiSamia=data.text.toLowerCase().includes('raisi samia');
    let ptype="POST";
    ...
    if(data.text.substring(0,2).toString()=="RT"){
      ...
      isRaisiSamia=getIncludes.tweets[pr_id].text.toLowerCase().includes('raisi samia');
    ...
    else if(isRaisiSamia){keyword='raisi samia';}
